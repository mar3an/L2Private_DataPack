...
public final class RimKamaloka extends AbstractInstance
{
	...
	protected synchronized final void enterInstance(L2PcInstance player, int index)
	{
		...
		else
		{
			...
			final int instanceId = InstanceManager.getInstance().createDynamicInstance("RimKamaloka.xml");
			...
			_log.info("Rim Kamaloka: started, Instance: " + instanceId + " created by player: " + player.getName());
			...
			ThreadPoolManager.getInstance().scheduleGeneral(new LockTask(world), LOCK_TIME * 60000);
